...
public class ShuJuKuCaoZuo extends ShuJuKuLianJie{
    ...
    public void PingLunNUMS(String tieziID,int pinglunNUMS) throws SQLException, ClassNotFoundException {
        TieZiLianJie tiezilianjie=new TieZiLianJie();
        tiezilianjie.openDB();
        String sql="update tu_cao set PingLunNUMS=? where tieziID=?";
        tiezilianjie.ps =tiezilianjie.con.prepareStatement(sql);
        tiezilianjie.ps.setString(2, tieziID);
        tiezilianjie.ps.setInt(1,pinglunNUMS );
